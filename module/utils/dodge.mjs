(async () => {
  ...
  const handleWeaponSelection = async (weaponIndex) => {
    ...
    const acidArmor = actor.system.armor.acid.total;
    const fireArmor = actor.system.armor.fire.total;
    const frostArmor = actor.system.armor.frost.total;
    const lightningArmor = actor.system.armor.lightning.total;
    const magicArmor = actor.system.armor.magic.total;
...
`;
    ...
    if (magicArmor > 0) {
      armorText += `<tr><td>Magic Armor</td><td>${magicArmor}</td></tr>`;
    }
    ...
    await ChatMessage.create({
      ...
      flavor: `
    <h2>
      <img src="${weapon.img}" title="${
        weapon.name
      }" width="36" height="36" style="vertical-align: middle; margin-right: 8px;"> Dodge
    </h2>
    <p style="text-align: center; font-size: 20px;"><b>
      ${
        deflect && actor.system.dodgeDeflect
          ? "Deflect"
          : critSuccess
          ? "Critical Success!"
          : critFailure
